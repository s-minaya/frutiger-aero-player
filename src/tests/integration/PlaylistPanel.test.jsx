...
describe("PlaylistPanel", () => {
  ...
  it("filtra playlists por nombre al escribir", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Filtrar playlists/i), {
      target: { value: "Rock" },
    });

    expect(screen.getByText("Rock clásico")).toBeInTheDocument();
    expect(screen.queryByText("Mi playlist favorita")).not.toBeInTheDocument();
  });
  ...
  it("llama a playTrack al hacer click en una canción de la playlist", async () => {
    render(
      <PlayerProvider>
        <PlaylistPanel isPremium={true} />
      </PlayerProvider>,
